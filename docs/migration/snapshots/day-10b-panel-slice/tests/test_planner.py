...
class SystemPromptTests(unittest.TestCase):
    ...
    def test_prompt_includes_panel_example(self):
        # Day 10b: the panel worked-example teaches the planner that a
        # panel debate ends with panel_agent (terminal), not synthesizer.
        from src.core.agents.registry import REGISTRY
        prompt = _build_system_prompt(_flags(), REGISTRY)
        self.assertIn("Example C", prompt)
        self.assertIn("panel_agent", prompt)

    def test_worked_examples_pass_schema(self):
        # The worked examples shown to the planner LLM must themselves be
        # schema-valid — otherwise we teach the model invalid output (e.g.
        # a bad estimated_complexity enum), costing a retry every call.
        ...
        for ex in (_EXAMPLE_SIMPLE_STOCK, _EXAMPLE_CLAIM_TRACKING, _EXAMPLE_PANEL):
            Plan.model_validate(ex)  # raises ValidationError if invalid

    def test_panel_example_is_registry_valid_and_terminal(self):
        # Day 10b: the panel example must pass registry validation with
        # the panel flag set, end with panel_agent, and contain no
        # synthesizer (panel writes its own report).
        from src.core.agents.registry import REGISTRY
        from src.core.types import Plan
        from src.core.planner import _EXAMPLE_PANEL
        plan = Plan.model_validate(_EXAMPLE_PANEL)
        errors = REGISTRY.validate_plan(
            plan, _flags(wants_portfolio_data=True, wants_panel_debate=True),
        )
        self.assertEqual(errors, [])
        agents = [s.agent for s in plan.steps]
        self.assertEqual(agents[-1], "panel_agent")
        self.assertNotIn("synthesizer", agents)
