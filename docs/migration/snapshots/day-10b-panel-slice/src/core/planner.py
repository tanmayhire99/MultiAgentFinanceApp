...
5. **The LAST step writes the user-visible report.** Normally that is \
a ``synthesizer`` step with empty ``tool_subset`` and ``depends_on`` \
referencing every step whose output the report needs. **EXCEPTION:** \
when ``wants_panel_debate`` is True, end with ``panel_agent`` instead \
— the panel writes its own debate transcript + closing brief, so do \
NOT add a ``synthesizer`` step after it.
...
### Example C — investor panel debate on a portfolio
User query: "What would the investor panel make of my portfolio?"
...
_EXAMPLE_PANEL = {
    "schema_version": "1.0",
    "goal": "Run the Buffett / Wood / Graham investor panel over the user's portfolio.",
    "rationale": "Panel debate requested - fetch the portfolio first, then panel_agent debates it and writes its own debate + closing brief. panel_agent is the terminal step; no synthesizer is added after it.",
    "estimated_complexity": "high",
    "steps": [
        {
            "id": 1,
            "description": "Fetch the user's holdings, sector allocation, concentration risks, and diversification score.",
        ...
        {
            "id": 2,
            "description": "Run the multi-round investor panel debate over the portfolio from step 1, then write the closing brief.",
            "agent": "panel_agent",
            "tool_subset": [],
            "depends_on": [1],
            "max_tool_calls": 0,
...
def _build_system_prompt(
    ...
    return _PLANNER_SYSTEM_TEMPLATE.format(
        ...
        example_panel=json.dumps(_EXAMPLE_PANEL, indent=2),
    )
