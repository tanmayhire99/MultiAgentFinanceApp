...
async def _run_one_step(
    *,
    ...
    #
    # Agents that produce rich live output (the panel debate) expose
    # ``run_streaming()``: an async generator that yields user-visible
    # PanelEvents as work happens and ends with a single
    # ``{"type": "_step_result", "result": StepResult}`` sentinel. We
    # forward the events to the caller in real time and commit the
    # sentinel's result. Every other agent uses the buffered ``run()``.
    # Detect streaming support on the *class* (an async-generator
    # function), not the instance. ``hasattr(agent, "run_streaming")``
    # is True for any MagicMock, which would wrongly route buffered
    # test doubles through the streaming branch; checking the class for
    # a real async-gen function is MagicMock-proof.
    streams_live = inspect.isasyncgenfunction(
        getattr(type(agent), "run_streaming", None)
    )
        ...
        if streams_live:
            ...
            if result is None:
                result = StepResult(
                    ...
                    status="failed",
                    output=None,
                    error="streaming agent produced no _step_result sentinel",
                    error_type="ExecutorStreamError",
                    started_at=time.time(),
                    completed_at=time.time(),
                )
        else:
