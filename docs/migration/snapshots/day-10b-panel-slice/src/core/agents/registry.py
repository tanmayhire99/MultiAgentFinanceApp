...
SYNTHESIZER = AgentDefinition(
    ...
    role_hint=(
        "every multi-step plan should end with this agent — except "
        "panel-debate plans, which end with panel_agent instead"
    ),
...
PANEL_AGENT = AgentDefinition(
    ...
    description=(
        ...
        "ticker or portfolio (persona sub-agents on a shared scratchpad, "
        "sequential rounds, convergence detection). Heavy: ~60-180s. "
        "Writes its own transcript + closing brief, so it is the TERMINAL "
        "step — never add a synthesizer after it. Include only when the "
        "classifier set wants_panel_debate."
    ),
    role_hint=(
