...
class PanelScopedAgent(ScopedAgent):
    ...
    async def run(self) -> StepResult:  # type: ignore[override]
        ...
        Buffered entry point used by tests and by any caller that does
        not want live events. Implemented by draining
        :meth:`run_streaming` and returning its terminal StepResult, so
        the streaming and buffered paths can never diverge.
        """
        result: Optional[StepResult] = None
        async for ev in self.run_streaming():
            if isinstance(ev, dict) and ev.get("type") == "_step_result":
                result = ev.get("result")
        if result is None:  # pragma: no cover - run_streaming always emits one
            return StepResult(
                step_id=self.step.id,
                status="failed",
                output=None,
                error="panel run_streaming produced no _step_result sentinel",
                error_type="PanelStreamError",
                started_at=time.time(),
                completed_at=time.time(),
            )
        return result

    async def run_streaming(self) -> AsyncIterator[Dict[str, Any]]:
        """Stream the debate + closing brief live, then the StepResult.

        Yields the debate's ``header`` / ``text`` events as each persona
        speaks (so the executor can forward them to the user in real
        time), then the ``Closing Brief`` section, and finally a single
        ``{"type": "_step_result", "result": StepResult}`` sentinel the
        executor consumes to commit the step to the scratchpad.

        Every exception is caught and surfaced as a ``failed``
        StepResult sentinel — nothing propagates to the executor. The
        StepResult produced here is identical to what :meth:`run`
        returns (``run`` simply drains this generator), so the existing
        buffered-path tests continue to exercise the same logic.
        """
        started_at = time.time()
        ...
        try:
            ...
            yield self._result_sentinel(StepResult(
                step_id=self.step.id,
                status="failed",
                output=None,
        ...
        # --- Phase 1: stream the debate live, accumulating a transcript ---
        yield {
            "type": "header",
            ...
            async for ev in run_debate_loop(
                ...
                if ev_type in ("header", "text"):
                    ...
                    if isinstance(text, str) and text:
                        ...
                        yield ev
                    continue
                # Non-renderable debate events (persona_verdict, etc.) are
                # forwarded so a richer UI can use them, but they do not
                # enter the transcript markdown.
                yield ev
        except Exception as exc:
        ...
        # --- Phase 2: closing brief (one moderator-synthesis call) ---
        try:
            ...
            # Soft-fail: we still have the transcript, so the panel
            # produces a complete-but-incomplete result rather than
        ...
        yield {
            "type": "text",
            "text": (closing_brief or "_(no closing brief was produced)_") + "\n",
            "persona": "moderator",
        }

        # --- Phase 3: assemble the StepResult (identical to the old run) ---
    ...
    @staticmethod
    def _result_sentinel(result: StepResult) -> Dict[str, Any]:
        """Wrap a StepResult in the ``run_streaming`` terminal sentinel.

        The executor (and :meth:`run`) recognise ``type ==
        "_step_result"`` and commit ``result`` to the scratchpad; the
        sentinel is never rendered to the user.
        """
        return {"type": "_step_result", "result": result}
