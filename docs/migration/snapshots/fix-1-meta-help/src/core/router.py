...
async def classify_query(
    ...
    """
    # Deterministic fast-path for obvious meta-questions about FinAI
    # itself. This is a PERFORMANCE optimisation, NOT a semantic decision:
    # the LLM still classifies any non-matching query, including subtler
    # meta-phrasings. We short-circuit only the unambiguous cases so
    # "what can you do?" returns instantly without spinning up the model.
    meta_decision = _detect_meta_help(query)
...
def _safe_fallback(
    ...
    *,
    ...
    """
    q = query.strip()
    # Even in the fallback path, prefer the meta-help fast-path. If the
    # LLM crashed entirely, "what can you do?" should STILL route to the
    # static capabilities answer rather than the educational generic-AI
    # hallucination we used to ship.
    meta_decision = _detect_meta_help(q)
    if meta_decision is not None:
        # Surface the fallback context in the rationale so the decision
        # card is honest about why we got here.
        meta_decision["rationale"] = (
            f"Fallback fast-path: meta-help phrase matched ({reason})."
        )
        return meta_decision
