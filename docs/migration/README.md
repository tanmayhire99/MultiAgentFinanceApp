...
| 10b | [Panel-slice planner examples + enable `/planner` prefix](DAY_10B_PANEL_SLICE.md) — adds `_EXAMPLE_PANEL_PORTFOLIO` and `_EXAMPLE_PANEL_STOCK` few-shot plans; enables `FINAI_PLANNER_PREFIX=1` by default | **`migration/day-10b-panel-slice`** | 1 file modified |
