...
def _format_system_prompt(persona: PersonaDef) -> str:
    """Append the shared output contract to every persona's system prompt."""
    return (
        f"{persona.system_prompt.strip()}\n\n"
        "### Output contract\n"
        "1. Use the available MCP tools to fetch any data you need. "
        "Prefer 2-4 focused tool calls over one broad fetch.\n"
        "2. After gathering the facts, write a short, first-person analysis in "
        "your own voice (150-250 words). Reference specific numbers from the "
        "tools you called.\n"
