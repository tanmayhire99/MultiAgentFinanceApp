...
ScopedAgent for a given :class:`~src.core.types.PlanStep`. The
executor doesn't know (and shouldn't care) which agent it's running
— it asks ``build_scoped_agent_for_step(step=..., ...)`` and gets
back a configured :class:`~src.core.agents._base.ScopedAgent` (or,
for the panel agent, a
:class:`~src.core.agents.panel_agent.PanelScopedAgent` subclass).

Adding a new agent
------------------
1. Register the agent in :mod:`src.core.agents.registry`.
2. Create ``src/core/agents/<name>_agent.py`` (or ``<name>.py`` —
   the synthesizer is named after its registry entry) with a
   ``build_<name>_agent`` factory function.
3. Import the factory at the top of THIS file and add an entry to
   :data:`_FACTORY_MAP`.
4. Add a focused unit test in ``tests/test_factories.py``.

The cross-cutting test
``CrossCuttingTests.test_factory_map_covers_every_registered_agent``
asserts that :data:`_FACTORY_MAP` keys exactly equal
``{a.name for a in REGISTRY}`` — so a new registry agent without a
factory (or vice versa) surfaces immediately in CI.

Why a tiny dedicated module
---------------------------
The dispatch logic (look up factory, raise on missing, forward
kwargs) is small but stable. Putting it in its own file means:

* Each per-agent file (``research_agent.py``, ``us_stock_agent.py``,
  …) stays focused on its own factory function, with no coupling to
  the other agents.
* The set of all agents is visible at one glance in
  :data:`_FACTORY_MAP` below, which is the canonical "what agents
  does this system know about?" answer.
* Tests can import ``_FACTORY_MAP`` directly to assert the
...
# factory function. Keys MUST equal ``{a.name for a in REGISTRY}`` —
# the cross-cutting test enforces this so a new registry agent without
# a factory (or vice versa) surfaces immediately in CI.
...
def build_scoped_agent_for_step(
    *,
    ...
    naming an agent that exists in neither the registry nor the
    factory map - the error message lists every known factory so the
    planner can self-correct on the next retry.
    """
