...
class ScopedAgent:
    ...
    def _build_system_prompt(self) -> str:
        ...
        return (
            ...
            "Your output is rendered by a strict GitHub-Flavored Markdown "
            "renderer, so malformed syntax shows up as literal `#` or `|` "
            "characters in the user's chat. Follow these rules exactly:\n"
            "- **Blank line before every table.** A `|` row must start on "
            "its own line, preceded by a blank line. Never run a label or "
            "sentence directly into a table — `**Label:**| a | b |` will "
            "render the pipes literally.\n"
            "- **Space after `#` in headings.** Write `### Section`, never "
            "`###Section`. CommonMark requires the space; without it the "
            "hashes render literally.\n"
            "- **Blank line before and after a heading** so the renderer "
            "can detect the block boundary.\n"
            "- **Tables need a separator row.** The line immediately "
            "after the header row must be `|---|---|` (one dash cell per "
            "column). GFM will not infer it.\n"
            "- **No `_italic_` for lines containing `|`, `{`, `}`, `[`, "
            "or `:`** — those characters break the underscore delimiter. "
            "Use `*italic*` or backticks instead.\n"
        )
