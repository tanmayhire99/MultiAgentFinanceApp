...
5) educational — concept / definition question about FINANCE. No market data needed.
   Signals: "explain", "what is X", "how does X work" where X is a finance/economics concept (compound interest, beta, EBITDA, etc.). NOT for questions about THIS application.
   "teach me", "difference between".

6) meta_help — user is asking about THIS application itself, not the market or a concept.
   Signals: "what can you do", "what are your capabilities", "what are you", "who are you", "how does this work" (referring to the app), "what is this", "help me", "tell me about yourself", "tell me about this app", "what features do you have", "/help".
   NOT educational — those are concept questions about finance. meta_help is meta-questions about FinAI.

7) smalltalk — user sent a casual / conversational message with no analytical intent. Greetings, thanks, acknowledgments, goodbyes.
   Signals: "hi", "hello", "hey", "good morning", "thanks", "thank you", "ok", "got it", "cool", "bye", "see you".
   Pick this when the message is < 6 words AND has no ticker, no concept word, no portfolio reference, no question about FinAI's features. If the message LOOKS like a greeting but has substantive content after it ("hi, what is beta?"), classify by the substantive part.
...
async def classify_query(
    ...
    """
    # Deterministic fast-paths for obvious cases. These are PERFORMANCE
    # optimisations, NOT semantic decisions: the LLM still classifies any
    # non-matching query, including subtler phrasings. We short-circuit
    # only the unambiguous cases so "hi" / "what can you do?" return
    # instantly without spinning up the model.
    fast_decision = _detect_meta_help(query) or _detect_smalltalk(query)
...
# Patterns that ONLY ever mean "user is asking about the FinAI app itself".
# Each pattern is deliberately narrow — partial / subtle phrasings like
# "what about your features" or "give me a tour" are intentionally NOT
# matched here so the LLM can apply its semantic judgement. False
# positives in this fastpath would route a real finance question to the
# capabilities answer, which is much worse than a slightly slower LLM call.
_META_HELP_PATTERNS = (
    # "what can / could / will you do" (with optional "for me")
    re.compile(r"\bwhat\s+(?:can|could|will|would)\s+you\s+do\b", re.IGNORECASE),
    # "what (are|is) your capabilit-y/-ies" - tolerates common misspellings
    re.compile(r"\bwhat\s+(?:are|is)\s+your\s+capabilit", re.IGNORECASE),
    # "what (do|are) you do" - generic "what do you do"
    re.compile(r"\bwhat\s+(?:do|does)\s+you\s+do\b", re.IGNORECASE),
    # "who / what are you" as a (near-)standalone question
    re.compile(r"^\s*(?:hi|hey|hello)?\s*[,!.\s]*\s*who\s+are\s+you\b", re.IGNORECASE),
    re.compile(r"^\s*(?:hi|hey|hello)?\s*[,!.\s]*\s*what\s+are\s+you\b(?!\s+(?:doing|analy|recommend))", re.IGNORECASE),
    # "tell me about (yourself | this app | finai)"
    re.compile(r"\btell\s+me\s+about\s+(?:yourself|this\s+(?:app|tool|assistant|bot|system)|finai)\b", re.IGNORECASE),
    # "how does this (app|work)" - app-relative, not concept-relative
    re.compile(r"\bhow\s+does\s+(?:this|finai)\s+(?:work|app|tool)\b", re.IGNORECASE),
    # bare "/help" or "help me" with nothing else
    re.compile(r"^\s*/?help\s*$", re.IGNORECASE),
    re.compile(r"^\s*help\s+me\s*$", re.IGNORECASE),
    # "what (is|are) (this|finai)"
...
def _detect_meta_help(query: str) -> Optional[RouteDecision]:
    """Return a meta_help RouteDecision if ``query`` is an obvious meta question.

    None means "let the LLM classify". A non-None return short-circuits
    the LLM call. See :data:`_META_HELP_PATTERNS` for the exact phrasings
    matched - we deliberately keep this narrow so the LLM still runs for
    the long tail.
    """
    q = query.strip()
    if not q:
        return None
    for pattern in _META_HELP_PATTERNS:
        if pattern.search(q):
            return {
                "intent": "meta_help",
                "tickers": [],
                "topic": "FinAI capabilities",
                "want_panel": False,
                "rationale": (
                    "Deterministic fast-path: query matches a meta-help "
                    "phrase (skipping the LLM router for speed)."
                ),
            }
    return None


# Patterns that ONLY ever mean "casual greeting / thanks / ack / goodbye".
# Same caveat as ``_META_HELP_PATTERNS``: keep these narrow. A false
# positive here routes a real finance question to a one-liner reply,
# which is much worse than a slightly slower LLM call. Each pattern
# anchors to start-of-string and rejects trailing substantive content -
# "hi, what is beta?" must NOT match here (the comma + content makes it
# a finance question with a greeting prefix).
_SMALLTALK_PATTERNS = (
    # Time-of-day greetings, standalone
    re.compile(
        r"^\s*(?:good\s+)?(?:morning|afternoon|evening|night)\s*[!.,?]*\s*$",
        re.IGNORECASE,
    ),
    # Bare greetings - "hi", "hey", "hello", "yo", "sup", with optional
    # punctuation and nothing else after.
    re.compile(
    ...
    # Thanks (standalone)
    re.compile(
        r"^\s*(?:thanks(?:\s+a\s+lot)?|thank\s+you|thx|ty|cheers|"
        r"appreciate\s+(?:it|that|you))\s*[!.,?]*\s*$",
        re.IGNORECASE,
    ),
    # Acknowledgments
    re.compile(
        r"^\s*(?:ok|okay|got\s+it|alright|cool|nice|sounds?\s+good|"
        r"makes?\s+sense|sure|right|yeah|yep|yup)\s*[!.,?]*\s*$",
        re.IGNORECASE,
    ),
    # Goodbyes
    re.compile(
...
def _detect_smalltalk(query: str) -> Optional[RouteDecision]:
    """Return a smalltalk RouteDecision if ``query`` is an obvious greeting.

    Symmetric with :func:`_detect_meta_help` - narrow regex set, returns
    None for any non-matching query so the LLM router can still apply
    semantic judgement on subtler casual phrasings (which the smalltalk
    flow itself handles via a small LLM fallback).
    """
    q = query.strip()
    if not q:
        return None
    for pattern in _SMALLTALK_PATTERNS:
        if pattern.search(q):
            return {
                ...
                "want_panel": False,
                "rationale": (
                    "Deterministic fast-path: query matches a smalltalk "
                    "phrase (skipping the LLM router for speed)."
                ),
            }
    return None
...
def _safe_fallback(
    ...
    *,
    ...
    """
    q = query.strip()
    # Even in the fallback path, prefer the deterministic fast-paths. If
    # the LLM router crashed entirely, "hi" / "what can you do?" should
    # STILL route correctly rather than landing in the educational
    # generic-AI hallucination we used to ship.
    fast_decision = _detect_meta_help(q) or _detect_smalltalk(q)
    if fast_decision is not None:
        # Surface the fallback context in the rationale so the decision
        # card is honest about why we got here.
        fast_decision["rationale"] = (
            f"Fallback fast-path: {fast_decision['intent']} phrase matched ({reason})."
        )
        return fast_decision
...
def render_decision_card(decision: RouteDecision, query: str) -> str:
    ...
    # The footer text honestly describes how the routing was picked. The
    # fast-path branch tells the audience "no LLM call here" so the
    # observed sub-second latency makes sense.
    if "fast-path" in rationale.lower():
        footer = (
            "_A deterministic fast-path matched this query (no LLM call), "
            f"so it routed directly to `{intent}`. Subtler phrasings "
            "would still go through the LLM router._"
        )
    else:
        footer = (
            f"_A small LLM call picked this flow from {len(INTENTS)} options; "
            "a different query (or follow-up) would have taken a different "
            "route through the agent graph._"
        )
